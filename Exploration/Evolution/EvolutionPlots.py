...
class DrawItem2(pg.GraphicsObject):
    ...
    def draw_rings(self, painter, alphabet, num_rings):
        ...
        for d, char in enumerate(alphabet):
            x, y = pol2cart(d / len(alphabet) * 2 * np.pi, 100)
    ...
    def draw_genomes(self, alphabet, data, painter):
        ...
        max_a = data.max(axis=1)
        la = len(alphabet)
        score_diff = (max_a[-1] - min_a[-1])

        if la > 0:
            for j in range(masked_data.shape[1]):
                if score_diff > 0:
                    c = int(255.0/score_diff*(masked_data[-1, j]-min_a[-1]))
                else:
                    c = 0
                painter.setPen(pg.mkPen(color=(255-c, c, 0, 255)))
    ...
    def draw_clicked_genome(self, alphabet, data, painter, clicked_id):
        ...
        painter.setPen(pg.mkPen(color=(0, 0, 255, 255)))
        self.draw_genome(painter, alphabet, len(alphabet), min_a, max_a, data[:, clicked_id])

    def update_pic(self, alphabet, data, clicked_id): #, groups, alphabet, p0, p1, p2, p3, p4, p5, nui, statistics, show_weights):
        ...
        self.draw_genomes(alphabet, data, painter)
            ...
            if tab.data is not None and tab.gene_keys is not None:
                ...
                if len(tab.clicked_id) > 0:
                    ...
                    if len(tab.clicked_id) > 0:
                        ...
                        tab.draw_item.update_pic(tab.gene_keys, tab.data, tab.clicked_id)
...
def update_evolution_plot(ui, tab, evo_name, gene_keys, data_folder=get_data_folder()):
    ...
    tab.draw_item.update_pic(gene_keys, tab.data, tab.clicked_id)
