...
class hist_tab():
    ...
    def initialize(self, Network_UI):
        self.additionaltab = Network_UI.Next_Tab('hist')
    ...
    def update(self, Network_UI):
        if self.additionaltab.isVisible():
            ...
            if hasattr(group, 'output'):
                self.neuron_act_data = group['n.output', 0, 'np'][-5000:, Network_UI.neuron_select_id]
                self.isi_plt.clear()
                ...
                avg_acts = np.mean(group['n.output', 0, 'np'][-Network_UI.x_steps:, not_input_mask], axis=0)#1000
                ...
                if input_mask is not False:
                    input_avg_acts = np.mean(group['n.output', 0, 'np'][-Network_UI.x_steps:, input_mask], axis=0)#1000
            ...
            gaba_syns = nsg['GABA']
            if len(gaba_syns) > 0:
                GABA_syn = Network_UI.get_combined_syn_mats(gaba_syns)
                GABA_syn = GABA_syn[list(GABA_syn.keys())[0]]
                selected_neuron_GABA_syn = GABA_syn[Network_UI.neuron_select_id]
