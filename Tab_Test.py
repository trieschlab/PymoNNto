...
class MyUITab(TabBase):
    ...
    def add_recorder_variables(self, net_obj, Network_UI):
        if hasattr(net_obj, 'activity'):
            Network_UI.add_recording_variable(net_obj, 'np.mean(n.activity)', timesteps=1000)

    def initialize(self, Network_UI):
        self.my_Tab = Network_UI.Next_Tab(self.title)
        self.my_curve = Network_UI.Add_plot_curve(x_label='t', y_label='mean activity')

    def update(self, Network_UI):
        if self.my_Tab.isVisible():
            data = Network_UI.network['np.mean(n.activity)', 0, 'np'][-1000:]
