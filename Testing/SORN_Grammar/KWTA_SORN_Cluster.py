...
def run(attrs={'name': 'KWTA', 'ind': [], 'N_e': 900, 'TS': [1], 'ff': True, 'fb': True, 'plastic': 15000}):
    ...
    e_ng = NeuronGroup(net=SORN, tag='PC_{},prediction_source'.format(1), size=get_squared_dim(attrs['N_e']), behaviour={
                2: SORN_init_neuron_vars_decay(timescale=1),
                ...
                21: SORN_STDP(eta_stdp='[0.00015#7]', STDP_F={-1: 1, 0: 1, 1: -1}),
    ...
    for i in range(1):
        ...
        sm.save_param_dict(attrs)
        score += train_and_generate_text(SORN, plastic_steps, 5000, 1000, display=print_info, stdp_off=True, same_timestep_without_feedback_loop=True, steps_recovery=0, storage_manager=sm)
