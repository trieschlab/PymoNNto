...
from NetworkBehaviour.Input.Music.MusicActivator import *
...
def run(tag, ind=[], par={'N_e':2000, 'TS':[1], 'ts_plastic':1000, 'ts_train':1000, 'ts_test':100}):

    assert tag in ['mono', 'poly']
    name = 'MelodicTrack'+'_'+tag+str(par['N_e'])+'N_e_'+str(par['ts_plastic'])+'_'+str(par['ts_train'])+str(par['TS'])

    sm = StorageManager(main_folder_name=tag, folder_name=name, random_nr=False, print_msg=display)
    sm.save_param_dict(par)

    if tag == 'mono':
        source = MonophonicActivator(tag='music_act', input_density=0.01, which_alphabet='train', beat_resolution=8)#output_size=par['N_e']
    else: 
        source = PolyphonicActivator(tag='music_act', input_density=0.01, which_alphabet='train', beat_resolution=4, instrument=73)

    SORN = Network()

    for lag in par['TS']:#
        e_ng = NeuronGroup(net=SORN, tag='exc_cell_{},prediction_source,text_input_group'.format(lag), size=get_squared_dim(int(par['N_e'])), behaviour={
            2: SORN_init_neuron_vars(timescale=lag, init_TH='0.1;+-100%'),
            ...
            21: SORN_STDP(eta_stdp='[0.0001#5]', STDP_F={-1: 1, 1: -1}),
        ...
        i_ng = NeuronGroup(net=SORN, tag='inh_cell_{}'.format(lag), size=get_squared_dim(int(0.2 * (par['N_e']))), behaviour={
            2: SORN_init_neuron_vars(timescale=lag, init_TH='0.1;+-0%'),
    ...
    SORN = run_plastic_phase(SORN, steps_plastic=1000, display=True, storage_manager=sm)

    readout, X_train, Y_train, X_test, Y_test = train_readout(SORN, source=SORN['music_act',0], steps_train=1000, steps_test=500, display=True, stdp_off=True, storage_manager=sm)
    
    score_predict_train = get_score_predict_next_step(SORN, SORN['music_act', 0], readout, X_train, Y_train, display=True, stdp_off=True, storage_manager=sm)

    score_predict_test = get_score_predict_next_step(SORN, SORN['music_act', 0], readout, X_test, Y_test, display=True, stdp_off=True, storage_manager=sm)
    
    score_spont = get_score_spontaneous_music(SORN, SORN['music_act', 0], readout, steps_spont=1000, display=True, stdp_off=True, 
    ...
    if tag=='mono':
        plot_specific_performance(score_predict_train, path=sm.absolute_path+'train_', title='predict training set, {} exc'.format(par['N_e']))
        plot_specific_performance(score_predict_test, path=sm.absolute_path+'test_', title='predict test set, {} exc'.format(par['N_e']))
        plot_frequencies(score_spont, path = sm.absolute_path+'frequencies', title='frequencies spontaneous output, {} exc'.format(par['N_e']))

    if tag=='poly':
        plot_frequencies_poly(score_spont, path=sm.absolute_path+'frequencies_notes', title='note frequencies spontaneous output, {} exc'.format(par['N_e']))
        plot_frequencies_chords_poly(score_spont, path=sm.absolute_path+'frequencies_chords', title='chord frequencies spontaneous output, {} exc'.format(par['N_e']))
...
if __name__ == '__main__':
    ...
    res_train, res_test, res_spont = run(tag='poly', 
    ind=[0.95, 0.4, 0.1383, 0.1698, 0.1, 0.00015, 0.04, 0.2944, 0.0006, 0.2, 0.015, 0.2944, 0.1, 0.0001, 0.87038, 0.82099, 1.5, 0.08, 15.0])
