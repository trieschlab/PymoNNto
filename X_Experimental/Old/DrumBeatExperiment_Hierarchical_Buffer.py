...
def run(tag, ind=[], par={'N_e':[1800], 'TS':[1]}):
    ...
    source = DrumBeatActivator(tag='drum_act', which_tracks=[100], THR_similar_tracks=0.5, input_density=0.015, offtoken=True, ontoken=True, include_inverse_alphabet=True)#, include_inverse_alphabet= True)#output_size=par['N_e']
    ...
    readout, X_train, Y_train, X_test, Y_test = train_readout(SORN, steps_train=20000, steps_test=100, source = SORN['drum_act', 0], display=True, stdp_off=True, storage_manager=sm)
...
if __name__ == '__main__':
    ...
    res_train, res_test, res_spont = run(tag='drum', ind=[0.95, 0.4, 0.1383, 0.1698, 0.1, 0.00015, 0.04, 0.2944, 0.0006, 0.2, 0.015, 0.2944, 0.1, 0.0001, 0.87038, 0.82099, 1.5, 0.08, 15.0])
